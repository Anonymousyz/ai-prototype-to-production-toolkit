...
| 人工复核与责任链 | 谁能批准、否决、升级、暂停?出错谁负责? | 复核设计、否决记录、决策负责人 |
...
flowchart TB
    ...
    dims --> V{"8 条否决任一为真?<br/><i>未授权用数 · 敏感数据入未批准模型 ·<br/>高风险动作无人工复核 · 无日志 ·<br/>无回滚负责人 · 输出质量不可评估 ·<br/>成本失控 · 演示冒充生产</i>"}
    V -- "是" --> STOP["不得推进:存在否决项<br/>(任何总分;退出码 1)"]
    V -- "否" --> T{"总分(归一化到 70)"}
    T -- "≤ 25" --> R1["仅限演示"]
    T -- "> 25, ≤ 45" --> R2["仅限受控试点"]
...
| [内部制度检索助手](examples/internal_policy_search_assistant.json) | 52/70,较强就绪度 |
| [客服自动操作代理](examples/customer_support_action_agent.json) | 触发两条否决,总分无效,退出码 1 |
